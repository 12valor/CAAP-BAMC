...
## Phase 8-10 archive, import, and portal boundaries

- The `employee-documents` bucket is private, restricted to PDF/JPEG/PNG, and
  capped at 50 MB. The application limit defaults to 25 MB through
  `documents.max_file_size_bytes`. Object paths use random UUID segments and
  contain no employee identifiers.
- Administrators prepare pending metadata before upload. Files above 6 MB use
  Supabase TUS resumable upload; every object is size- and signature-verified
  before becoming available. Preview/download routes issue five-minute signed
  URLs. Soft deletion retains the private object for restore.
- Workbooks are parsed in server memory and are not retained. Normalized rows,
  validation messages, and a SHA-256 digest form a preview. Explicit
  confirmation invokes one database transaction, so failed linked records roll
  back the complete job. Passwords are never accepted or generated by import.
- Employee overview and statement functions derive identity from `auth.uid()`
